...
const DefaulterCard = ({ d, urgColor, onPay, onSms }) => {
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div>
          <div style={{ fontWeight: 700, fontSize: 14 }}>{d.student_name}</div>
          <Mono color={C.subtle}>{d.admission_no}</Mono>
          <div style={{ fontSize: 12, color: C.muted, marginTop: 2 }}>{d.class_name || "—"}</div>
        </div>
        <span style={{ fontWeight: 700, color: C.rose, fontSize: 15, fontFamily: "'DM Mono',monospace" }}>
          KES {parseFloat(d.balance).toLocaleString()}
        </span>
      </div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: C.muted }}>
        <span>{invLabel}</span>
        <span style={{ fontWeight: 700, color: urg }}>{days}d overdue</span>
      </div>

      <Bar pct={Math.min((days / 90) * 100, 100)} color={urg} height={5} />

      <div style={{ display: "flex", gap: 8 }}>
        <Btn size="sm" variant="mpesa" style={{ flex: 1 }}
          onClick={() => onPay({ id: d.invoice_id, invoice_no: d.invoice_no, admission_no: d.admission_no, balance: parseFloat(d.balance) })}>
          Pay
        </Btn>
        <Btn size="sm" variant="outline" style={{ flex: 1 }} onClick={onSms}>SMS</Btn>
      </div>
    </div>
  );
};
...
export const DefaultersTab = ({ activeTerm }) => {
  ...
  const sorted   = [...defaulters].sort((a, b) => parseFloat(b.balance) - parseFloat(a.balance));
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {defaulters.length > 0 && (
        <div style={{
          ...
          <div style={{ width: 36, height: 36, borderRadius: 10, background: `${C.rose}20`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, flexShrink: 0 }}>⚠️</div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontWeight: 700, marginBottom: 2, fontSize: 13 }}>{defaulters.length} students with outstanding balances</div>
            <div style={{ fontSize: 12, color: C.muted }}>
              Total: <b style={{ color: C.rose, fontFamily: "'DM Mono',monospace" }}>KES {total.toLocaleString("en-KE")}</b>
            </div>
          </div>
          <Btn size="sm" variant="outline" onClick={() => setShowSms(true)}>📱 SMS Reminders</Btn>
        </div>
      )}
      ...
      {isMobile ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {loading ? (
            [0,1,2].map(i => (
              <div key={i} style={{ height: 140, borderRadius: 12, background: C.surface, border: `1px solid ${C.border}` }}>
                <div className="sk" style={{ height: "100%", borderRadius: 12 }} />
              </div>
            ))
          ) : sorted.length === 0 ? (
            <EmptyState title="No defaulters!" sub="All students are up to date" />
          ) : sorted.map((d, i) => (
            <DefaulterCard
              key={d.id || i}
              d={d}
        ...
        <Surface style={{ overflow: "hidden" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr>{["Student","Class","Invoice","Balance","Days Overdue","Urgency","Actions"].map(h => <TH key={h} label={h} />)}</tr>
            </thead>
            <tbody>
              {loading ? <SkeletonRows cols={7} /> : sorted.length === 0 ? (
